...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность = Истина Тогда
		...
		Иначе
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), "Данные изменены! Сохранить результат в файл", РежимДиалогаВопрос.ДаНет);	
		КонецЕсли;
...
Процедура ПриЗакрытии()
	Если ПоказыватьПланВыполненияЗапроса Тогда
		...
		ПоказыватьПланВыполненияЗапроса = 0;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ТипВФормеЗавершениеВыбора(ВыбранныйЭлемент,
...
Процедура ЗначениеВФормеПриИзменении(Элемент)
	...
	Если ТекущийТип <> "ТаблицаЗначений" И ТекущийТип <> "МоментВремени"
			И ТекущийТип <> "Граница" Тогда
...
&НаКлиенте
Процедура ЗначениеВФормеНачалоВыбора(Элемент, ДанныеВыбора,
...
Процедура ВыполнитьВыборРезультатаЗапроса(ппКоманда)
	Для Каждого СтрокаЗапроса Из Объект.Запросы Цикл
		Если СтрокаЗапроса.Идентификатор = ИдентификаторТекущегоЗапроса
				И Не ПустаяСтрока(СтрокаЗапроса.АдресРезультатовЗапроса) Тогда
			ОповеститьОВыборе(Новый Структура("ДействиеВыбора, ДанныеВыбора", Параметры.ДействиеВыбора, СтрокаЗапроса.АдресРезультатовЗапроса));
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПослеЗакрытияКонструктораЗапроса(Текст, ДопПараметры) Экспорт
	...
	ТекущийЗапрос = Объект.Запросы.Получить(ДопПараметры.ИндексТекущегоЗапроса
		- 1);
	Если ТекущийЗапрос.Имя = ИмяЗапросаПоУмолчанию Тогда
...
Функция ВыгрузитьНастройки(ПараметрыПередачи)
	...
	Объект.ЦветЗапрос				= ПолучитьИзВременногоХранилища(ПараметрыПередачи.АдресХранилища).ЦветЗапрос;
...
// Обновление параметров.
	...
	Если КоличествоЗапросов > 1 Тогда
		ЗаголовокКнопкиВыбораЗапроса = ЗаголовокКнопкиВыбораЗапроса + " ("
			+ КоличествоЗапросов + ")";
	КонецЕсли;
...
Процедура СохранитьЗапросы()
	...
	Если ПроверитьВозможностьЗаписиСпискаЗапросов() = Ложь Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Автосохранение невозможно.'"), , "Файл запросов создан в другой конфигурации!");
		Возврат;
...
// "Захватывается" текущий запрос из списка запросов.
	...
	Объект.Запросы.Получить(ИндексТекущегоЗапроса
		- 1).Текст = ОтформатированныйТекст;
...
//
&НаСервере
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса,
		ТабличныйДокументРезультата, ВыводитьВременныеТаблицы,
		ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения, Тонкий = Ложь)
...
&НаСервере
Процедура ПолучитьРезультатЗапросаИзХранилища(ТабличныйДокументРезультата,
		АдресРезультата)
...
Процедура СохранитьФайлЗапросаЗавершение(Оповещение) Экспорт
	...
	Если ПроверитьВозможностьЗаписиСпискаЗапросов() = Ложь Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СохранитьФайлЗапросаЗавершение1", ЭтаФорма, Новый Структура("Оповещение", Оповещение)), "Файл запросов создан в другой конфигурации!
			|Возможна потеря типов параметров!
			|Вы всё равно хотите записать файл?", РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, "Внимание!", КодВозвратаДиалога.Нет);
		Возврат;
...
&НаКлиенте
Процедура СохранитьФайлЗапросаЗавершение1(РезультатВопроса,
		ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьЗапросыИПараметрыИзОбъектаВнешнейОбработки(ОбъектОбработки)
	...
	Для каждого ТекПараметр Из ПараметрыОб Цикл
		...
		Если ТипСтрока = "ТаблицаЗначений" Или ТипСтрока = "МоментВремени"
				Или ТипСтрока = "Граница" Тогда
			ЭлементПараметр = Объект.Параметры.Добавить();
...
Процедура ОткрытьФормуПланаВыполненияЗапроса()
	...
	Если ЗначениеЗаполнено(МеткаЗапроса) Тогда
		...
		ОткрытьФорму(Объект.ПутьКФормам
			+ ".ПланВыполненияЗапроса", ПараметрыЗапроса, ЭтотОбъект, Истина);
	КонецЕсли;
...
// ПроверитьВозможностьЗаписиСпискаЗапросос()
&НаКлиенте
