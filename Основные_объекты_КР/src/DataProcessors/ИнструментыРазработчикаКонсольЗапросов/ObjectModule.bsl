#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Функция ВыгрузкаРезультата(РезультатЗапроса, ПорядокОбхода, Иерархия, Тонкий = Ложь)
		...
		Если Тонкий Тогда
			...
			Если ТипЗнч(ВыгруженноеЗначение) = Тип("ДеревоЗначений") Тогда
			...
			Для каждого ТекКолонка Из ВыгруженноеЗначение.Колонки Цикл
				...
				Если ТекКолонка.ТипЗначения = Новый ОписаниеТипов("ТаблицаЗначений") Тогда
					...
					ВыгруженноеЗначение.Колонки.Добавить((ТекКолонка.Имя + "_"), Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(0)));
				КонецЕсли;
				
			КонецЦикла;
			
			Для каждого ТекЗначение Из ВыгруженноеЗначение Цикл
				
				Для каждого ИмяКолонки Из МассивИменКолонокТЗ Цикл
				
					ТЗ = ТекЗначение[ИмяКолонки];
					КоличествоКолонокТЗ = ТЗ.Колонки.Количество();
					КоличествоСтрокТЗ = ТЗ.Количество();
					
					ТекЗначение[ИмяКолонки + "_"] = "";
					
					Для Сч = 1 По КоличествоКолонокТЗ Цикл
						
						ТекЗначение[ИмяКолонки + "_"] = ТекЗначение[ИмяКолонки + "_"] + ТЗ.Колонки[Сч - 1].Имя + " , ";
						
					КонецЦикла;
					
					Если КоличествоСтрокТЗ > 0 Тогда
					
						ТекЗначение[ИмяКолонки + "_"] = ТекЗначение[ИмяКолонки + "_"] + Символы.ПС;
					
					КонецЕсли;
		
					
					Для Инд = 1 По КоличествоСтрокТЗ Цикл
						
						Для Сч = 1 По КоличествоКолонокТЗ Цикл
						
							ТекЗначение[ИмяКолонки + "_"] = ТекЗначение[ИмяКолонки + "_"] + Строка(ТЗ[Инд - 1][Сч - 1]) + " , ";
						
						КонецЦикла;
						
						Если Инд <> КоличествоСтрокТЗ Тогда
			...
			Для каждого ТекИмяКолонки Из МассивИменКолонокТЗ Цикл
				
				ВыгруженноеЗначение.Колонки.Удалить(ТекИмяКолонки);
				КолонкаТЗ = ВыгруженноеЗначение.Колонки.Найти(ТекИмяКолонки + "_");
				КолонкаТЗ.Имя = ТекИмяКолонки;
